/**
 * 这个是用在pc浏览器端的，支持搜索添加list功能的播放器
 * 未完成
 * @author theowang
 * $Id: yplayer.js 464 2012-11-08 11:20:28Z ksky521@gmail.com $
 */
var playList = [
{
    title:'17岁 - 刘德华',
	author:'刘德华',
    lrc:'../audio/17.lrc',
	src:'http://storage2.fcdn.kugou.com/M00/31/58/Otfxxk55J3uKgZVYAA9l63SlV8Y351.m4a'
},
{
    title:'断桥残雪',
	author:'许嵩',
    lrc:'../audio/dqcx.lrc',
	src:'http://storage2.fcdn.kugou.com/M01/10/EA/Otfxxk5tB_eP76KgAA6J8Wc6pxo531.m4a'
},
{
    title:'套马杆 - 草原风',
    author:'乌兰图雅',
	lrc:'../audio/tmg.lrc',
	src:'http://storage3.fcdn.kugou.com/M00/07/FC/OtfxzE5u6UPM0qIkABHL7mur00s613.m4a'
},
{
    title:'最炫民族风',
	author:'凤凰传奇',
    lrc:'../audio/zxmzf.lrc',
	src:'http://storage2.fcdn.kugou.com/M01/17/98/Otfxxk5upsKRCNSxABIQZjHTdGY083.m4a'
}
];
...
;(function(){
	...
	var isMp3 = !! (document.createElement('audio').canPlayType("audio/mpeg"));
...
//              console.log(i);
	 ...
	 */
    function loadSong(i) {
		var source = playList[(i||curIndex)|0];
		var htmlstr = '',len = 0;
		
		
		htmlstr += '<audio id="playerAudio" preload="auto" >'+
                      '<source src="songProxy.php?url=' + encodeURIComponent(source.src) + '"></source>'+
                  '</audio>';
...
//        $title.html(data.join(' - '));
	...
	var Kugou = {
		...
		bindEvent: function(){
			addEvent($('searchForm'), 'submit', Kugou.searchEvent);
		},
		...
		getSongInfo: function(hash, callback){
			...
			var url = rootURL + '?base=/app/i/getSongInfo.php&query='+ encodeURIComponent('cmd=playInfo&hash=' + hash);
			get(url, callback);
		}
	};
