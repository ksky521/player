/**
 * 初始化只创建一次audio对象
 * 考虑到跨域问题，所以采用了代理方式获取歌曲内容
 * 不知道后面的chrome版本是否支持m4a跨域，如果支持可以去掉代理
 * 
 * @author theowang
 * $Id: player2.js 465 2012-11-08 11:23:53Z ksky521@gmail.com $
 */
var playList = [
{
    title:'17岁 - 刘德华',
...
;!function(){
	...
	function updateProgress(){
		...
		curTime += 100;//加快.1秒，为了快速响应
		  setProgress(curTime);
		 //此处如果切换到歌词才会同步到document.title，为了节省计算成本~
		 //如果不考虑可以将此处打开
		if (!isShowLrc()) {
			return;
		}
...
//              console.log(i);
    ...
    function loadSong(i) {
		...
		htmlstr += '<audio id="playerAudio" preload="auto" >'+
                  ...
                  '</audio>';
        //去除代理，这个在chrome下竟然不行！非要代理不成……好像是chrome20+加上的限制……
		// htmlstr += '<audio id="playerAudio" preload="auto" >'+
